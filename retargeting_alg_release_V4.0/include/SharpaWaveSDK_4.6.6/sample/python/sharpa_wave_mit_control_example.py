...
def initialize(hand) -> bool:
    error = hand.set_control_mode(ControlMode.MIT)
    if error.code != 0:
    ...
    # MIT control parameters: read then write
    error, param_json = hand.get_parameter(["mit_kp", "mit_kd"])
    if error.code != 0:
        print(f"Failed to get MIT params: {error.message}")
        return False
    try:
        params = json.loads(param_json)
        mit_kp = params.get("mit_kp")
        mit_kd = params.get("mit_kd")
        print(f"MIT params (read): mit_kp={mit_kp}, mit_kd={mit_kd}")
    except (json.JSONDecodeError, TypeError) as e:
        print(f"Failed to parse MIT params JSON: {e}")
        mit_kp, mit_kd = None, None

    # Write MIT params (use read values if valid, else defaults)
    # mit_kp_write = mit_kp if mit_kp is not None else 1.0
    # mit_kd_write = mit_kd if mit_kd is not None else 0.1
    # error = hand.set_parameter_safe(json.dumps({"mit_kp": mit_kp_write, "mit_kd": mit_kd_write}))
    # if error.code != 0:
    #     print(f"Failed to set MIT params: {error.message}")
    #     return False
    # print(f"MIT params (written): mit_kp={mit_kp_write}, mit_kd={mit_kd_write}")
...
def run_cycles(hand):
    ...
    for cycle in range(cycles):
        for tick in range(total_tick):
            ...
            # MIT control: position + feedforward velocity + feedforward torque
            # velocity_list = [(angle_list[i] - prev_angle_list[i]) / dt for i in range(22)]
            velocity_list = [0.0] * 22
            torque_list = [0.0] * 22
            error = hand.set_mit_control(angle_list, velocity_list, torque_list)
            if error.code != 0:
                print(f"Failed to set_mit_control: {error.message}")
            prev_angle_list = list(angle_list)
            time.sleep(dt)
