...
def main():
    ...
    try:
        ...
        # custom_config_path = "/path/to/your/tactile_config.json"
        # for i, wave in enumerate(waves):
        #     error = wave.set_tactile_config_file(custom_config_path)
        #     if error.code == 0:
        #         print(f"Custom tactile config set for device {i}: {custom_config_path}")
        #     else:
        #         print(f"Failed to set tactile config for device {i}: {error.message}")
        ...
        for i, wave in enumerate(waves):
            if not wave.start():
                print(f"Wave {i} startup failed: check any program on port {wave.get_device_info().tactile_data_config.target_port}")
            ...
            if key == ord('j'):
                ...
                for i, wave in enumerate(waves):
                    try:
                        ...
                        err = wave.set_tactile_jpeg_enable(True, -1)
                        if err.code == 0:
            ...
            if key == ord('s'):
                ...
                for i, wave in enumerate(waves):
                    try:
                        ...
                        err = wave.set_tactile_jpeg_enable(False, -1)
                        if err.code == 0:
